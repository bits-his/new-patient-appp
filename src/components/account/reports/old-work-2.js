        ...
        .then(data => {
            if(data.success){
                ...
                Object.keys(final).forEach(head => {
                    final[head] = [...final[head], { des: `Total ${head}`, 
                    subhead: final[head][0].subhead,
                    debit: final[head].reduce((a,b) => a + parseInt(b.debit), 0),
                    credit: final[head].reduce((a,b) => a + parseInt(b.credit), 0)
                }]
                })
                            ...
                            Object.keys(formattedFinancialStatement).map((head) => (
                                <>
                                    ...
                                    {formattedFinancialStatement[head].map((item, index) => (
                                        <tr key={index}>
                                            <td>{item.des}</td>
                                            <td className='text-right'>{item.subhead&&(item.subhead.toString().substr(0,4) === '5000') 
                                            ? '-' :
                                            formatNumber(Math.abs(item.debit))}</td>
                                            <td className='text-right'>{item.subhead&&item.subhead.toString().substr(0,4) === '4000' ? '-' :
