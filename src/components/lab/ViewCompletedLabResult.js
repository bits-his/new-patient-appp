...
import SampleForm from './NewLaboratory/SampleForm'
...
function ViewCompletedLabResults() {
  ...
  const getRequestList = () => {
    _fetchApi2(
      `${apiURL()}/lab/get-labs-by-condition?query_type=by_req_id&condition=${request_id}&facilityId=${
      ...
      .forEach((label) => {
        // console.log(label);
          ...
          for (let i = 0; i < label.noOfLabels; i++) {
            let dept =
              label.department === '2000'
                ? 'Hematology'
                : label.department === '3000'
                ? 'ChemPath'
                : label.department === '4000'
                ? 'Microbiology'
                : 'Radiology'
            label.dept = dept
  ...
  const printBarcode = () => {
    ...
    _fetchApi(`${apiURL()}/lab/get-barcodes?request_id=${request_id}`, data => {
      if(data && data.barcode) {
        // data.barcode.forEach((label) => {
          printLabels(data.barcode)
        // })
      }
  ...
  return (
    <>
      ...
      <Row>
        ...
        {isDoctorInterface ? null : previewMode ? (
          <Preview
            setPreviewMode={setPreviewMode}
            previewIsReceipt={previewIsReceipt}
...
export default ViewCompletedLabResults
